...
import { createMessage, getConversations, getMessages, getRestaurants, getUsers } from "../api/client";
...
export function ChatPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getUsers(),
      getRestaurants(),
    ]).then(([conversationResult, messageResult, userResult, restaurantResult]) => {
      if (!mounted) return;
      ...
      setSelectedConv((current) => current ?? conversationResult[0]?.id ?? null);
    });
